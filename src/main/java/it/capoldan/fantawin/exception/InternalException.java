package it.capoldan.fantawin.exception;

import org.springframework.http.HttpStatus;

/**
 * Eccezione di errore interno generico, viene tradotta in un errore 500
 * E' possibile crearla con un codice di errore opportuno
 */
public class InternalException extends RuntimeException {

    /**
     * @deprecated
     * Costruttore deprecato, usare quello che prevede il passaggio anche dell'errorCode
     *
     * @param message messaggio diagnostico
     */
    @Deprecated(since = "0.0.2-Snapshot")
    public InternalException(String message) {
        this(message, ExceptionsCodes.ERROR_CODE_GENERIC_ERROR, null);
    }

    /**
     * @deprecated
     * Costruttore deprecato, usare quello che prevede il passaggio anche dell'errorCode
     *
     * @param message messaggio diagnostico
     * @param cause eccezione causa
     */
    @Deprecated(since = "0.0.2-Snapshot")
    public InternalException(String message, Throwable cause) {
        this(message, ExceptionsCodes.ERROR_CODE_GENERIC_ERROR, cause);
    }

    public InternalException(String message, String errorCode) {
        this(message, errorCode, null);
    }

    public InternalException(String message, String errorCode, Throwable cause) {
        super(HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), message, HttpStatus.INTERNAL_SERVER_ERROR.value(), errorCode, null, null, cause);
    }
    
    public InternalException(String message, int httpStatus, String errorCode) {
        super(HttpStatus.valueOf(httpStatus).getReasonPhrase(), message, httpStatus, errorCode, null, null, null);
    }
}

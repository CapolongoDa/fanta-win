    ...
    private Classifier<Throwable, RetryPolicy> configureStatusCodeBasedRetryPolicy(int retryMaxAttempts) {
        ...
        return throwable -> {
            ...
            if (throwable instanceof HttpStatusCodeException httpException) {
                retryPolicy = getRetryPolicyForStatus(HttpStatus.valueOf(httpException.getStatusCode().value()), simpleRetryPolicy, neverRetryPolicy);
            }
            else if(throwable instanceof HttpResponseException pnHttpResponseException && pnHttpResponseException.getStatusCode() > 0) {
